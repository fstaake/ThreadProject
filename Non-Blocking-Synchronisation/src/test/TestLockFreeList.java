...
/**
 * Test case for the {@link LockFreeList}.
 * 
...
public class TestLockFreeList extends AbstractBenchmark {
	...
	/**
	 * Initialize the {@link LockFreeList}.
	 */
	@Before
	public void before() {
	...
	@BenchmarkOptions(benchmarkRounds = 500, warmupRounds = 0)
	@Test
	public void testLockFreeListAddRemove() {
		IntStream.rangeClosed(1, 500).parallel().forEach(this.lockFreeList::add);
		IntStream.rangeClosed(1, 500).parallel().forEach(this.lockFreeList::add);
		IntStream.rangeClosed(1, 500).parallel().forEach(this.lockFreeList::add);
		IntStream.rangeClosed(1, 500).parallel().forEach(this.lockFreeList::add);

		Assert.assertEquals(ADD_ADD, this.lockFreeList.toString());
	}
}
